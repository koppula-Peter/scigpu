...
module scigpu_mask_control_m5 #(
  ...
  localparam bit [LIW-1:0] LI_NONE = LIW'(DEPTH);
  ...
  wire [IW-1:0] top_idx = (sp_q[sl_q] != '0)
                        ? IW'(sp_q[sl_q] - SPW'(1)) : IW'(0);
  frame_t top_r, li_r;
  assign top_r = (sp_q[sl_q] != '0) ? frames[sl_q][top_idx] : '0;
  assign li_r  = ((li_q != LI_NONE) && (li_q < SPW'(DEPTH)))
...
`endif
          if (cbr_bad) begin
            ...
            c_wfen=1'b1; c_wfi=IW'(sp_q[sl_q]);
            c_spen=1'b1; c_spv=sp_q[sl_q]+SPW'(1);
  ...
  wire bc_direct   = (sp_q[sl_q] == (li_q + SPW'(1)));
        ...
        S_VALID: begin
          if (bc_dispatch) begin
            if (bc_no_loop) begin
              ...
              if (bc_direct) begin
                st_q<=S_SCRUB; wi_q<=SPW'(DEPTH); // sentinel: apply now
              end else begin
                wi_q<=sp_q[sl_q]-SPW'(1);
                st_q<=S_SCRUB;
              end
            end
...
`endif
          ...
          end else if (wi_q <= SPW'(li_q)) begin
        ...
        S_RETSCRUB: begin
          if (wi_q >= sp_q[sl_q]) begin
            ...
            wf_d.mask_a<=fr.mask_a  & ~retm_q;
            wf_d.mask_b<=fr.mask_b  & ~retm_q;
            wf_d.future<=fr.future  & ~retm_q;
            wf_en<=1'b1; wf_i<=wi_q[IW-1:0];
            wi_q<=wi_q+SPW'(1);
          end
        end
        ...
        S_UNWIND: begin
          if (wlv_q==32'b0) begin
            ...
            if (fr.ftype==FT_IF) begin
              if ((fr.mask_a & wlv_q)!=32'b0) begin
`ifdef SCIGPU_M5_DBG
                $display("M5ENGIN RESUME fr.pc_a=%0d pc_b=%0d sp=%0d",
                    fr.pc_a, fr.pc_b, sp_q[sl_q]);
`endif
                wf_d<=fr; wf_d.mask_a<='0; wf_d.phase<=1'b1;
                wf_en<=1'b1; wf_i<=top_idx;
                d_done_q<=1'b1;
                d_ex_q<=fr.mask_a & wlv_q; d_lv_q<=wlv_q;
                d_sp_q<=sp_q[sl_q]; d_li_q<=li_q;
                d_pc_q<=fr.pc_a; d_pcwe_q<=1'b1;
                st_q<=S_IDLE;
              end else begin
                sp_en<=1'b1; sp_v<=sp_q[sl_q]-SPW'(1);
                wex_q<=fr.parent & wlv_q;
                d_pc_q<=fr.pc_b+64'd1; d_pcwe_q<=1'b1;
                if ((fr.parent & wlv_q)!=32'b0) begin
                  d_done_q<=1'b1;
                  d_ex_q<=fr.parent & wlv_q; d_lv_q<=wlv_q;
                  d_sp_q<=sp_v; d_li_q<=li_q;
                  st_q<=S_IDLE;
                end
                // else remain in UNWIND and keep popping
              end
            end else if (fr.ftype==FT_LOOP) begin
              d_done_q<=1'b1; d_route_q<=1'b1;
              d_ex_q<=32'b0; d_lv_q<=wlv_q;
              d_sp_q<=sp_q[sl_q]; d_li_q<=li_q;
              d_pc_q<=fr.pc_b; d_pcwe_q<=1'b1;
              st_q<=S_IDLE;
            end else begin
              sp_en<=1'b1; sp_v<=sp_q[sl_q]-SPW'(1);   // MANUAL
              wex_q<=fr.parent & wlv_q;
              if ((fr.parent & wlv_q)!=32'b0) begin
                d_done_q<=1'b1;
                d_ex_q<=fr.parent & wlv_q; d_lv_q<=wlv_q;
...
`ifdef SCIGPU_FORMAL
  ...
  sp_bounds_check: assert property (@(posedge clk) disable iff (rst)
    (sp_q[dbg_slot] <= SPW'(DEPTH)));
`endif

  assign dbg_sp = sp_q[dbg_slot];
  wire [IW-1:0] dbg_ti = (sp_q[dbg_slot]!='0)
                       ? IW'(sp_q[dbg_slot]-SPW'(1)) : IW'(0);
  assign dbg_top_ftype  = (sp_q[dbg_slot]=='0) ? 2'd0 : frames[dbg_slot][dbg_ti].ftype;
  assign dbg_top_parent = (sp_q[dbg_slot]=='0) ? 32'b0 : frames[dbg_slot][dbg_ti].parent;
  assign dbg_top_maska  = (sp_q[dbg_slot]=='0) ? 32'b0 : frames[dbg_slot][dbg_ti].mask_a;
  assign dbg_top_maskb  = (sp_q[dbg_slot]=='0) ? 32'b0 : frames[dbg_slot][dbg_ti].mask_b;
