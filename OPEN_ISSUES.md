...
| OI-016 | FP32 vector ALU: rounding edge cases in subnormal handling and subtraction normalization produce incorrect results for certain operand patterns (VCVT works, FADD/FSUB/FMUL partially working). Root cause: multi-bit left-shift normalization loop in fp_add and sticky-bit propagation. | Closed: full IEEE 754 SP rewrite of scigpu_fp32_alu.sv — GRS adder w/ asymmetric on-grid-half RN-even, exact-product rounder, two-sided subnormal scaling, f2i byte-signedness fix, x+x cancellation-vs-carry fix. Unit TB 240424 checks PASS; core differential fp_arith/fp_consts kernels PASS; M1–M5 regression re-validated green after va_op[4:0] reconciliation. Evidence: reports/evidence/m7/m7_phase2_summary.md | directive §12-13; ISA-001 §V; reviews/M7 gate pending | 2026-08-25, M7 phase 2 |
...
| OI-014 | Random generator emits BREAK inside nested IF within loops whose multi-path drain/resume interplay shows golden-vs-RTL event-stream divergence in a residual seed set; generator currently restricts randomized BREAK to loop-body scope (nested case covered by directed break_if_scrub + golden G04). Root-cause analysis pending joint stepping harness. | Closed: residual-seed divergence root-caused 2026-08-25 — NOT mask-control semantics. decode_m5/decode_m3 never admitted V_MIN/V_MAX (0x110/0x111) into v_any legality, so generator kernels using them faulted illegal in RTL while golden executed; ~33% of random seeds affected post-M6-era generator evolution. Fix: vmin/vmax wires added to v_any + case dispatch (va_op 13/14) in both decoders. Campaigns after fix: random 997/997, crosswidth 248/248 (L4/L8/L16/L32), multiwf 250/250, reset clean — all zero mismatches. Evidence: reports/evidence/m5/{random_divergence,cross_width,random_multiwf,reset_stress}.log | directive §12-13; ISA-001 §16 | 2026-08-25, M7 phase 3 |
